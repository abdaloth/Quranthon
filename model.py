...
sample_transform = torchaudio.transforms.Resample(
    orig_freq=original_sample_rate, new_freq=target_sample_rate
)
mfcc_transform = torchaudio.transforms.MFCC(
    sample_rate=target_sample_rate, n_mfcc=n_mfcc, log_mels=log_mels
)
...
class Model(nn.Module):
    def __init__(
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            encoder_layer, num_layers, norm=normalization
        )
    ...
    def forward(self, features):
        ...
        output_reshaped = output_dropped.reshape(
            -1, clip_length * d_model
        )  # from (N,S,E) to (N,S*E)
...
model = Model(
    d_model, nhead, dim_feedforward, dropout_rate, activation_function, num_layers
)
model.load_state_dict(
    torch.load("static/model_state_dict", map_location=torch.device("cpu"))
)
